...
static double wheel_vels_to_curvature(double left, double right,
                                      double track_width) {
...
deserialize_pathfinder_path(std::istream &left, std::istream &right) {
  ...
  for (std::size_t i = 0; i < left_path->size(); ++i) {
    ...
    path[i] = ProfilePoint(ControlVector(pose, vel, accel, jerk), {l_v, r_v},
                           curvature, left_path.value()[i].time);
  }
