...
std::vector<ProfilePoint> SplineGenerator::_generate(Iter start, Iter end,
  ...
  for (auto vec = std::next(start); vec != end; ++vec) {
    ...
    auto profiled_path =
        parameterize(spline_start, spline_end, raw_path, preferred_start_vel,
                     preferred_end_vel, start_time);
...
SplineGenerator::gen_single_raw_path(ControlVector start, ControlVector end,
  ...
  for (auto time : times) {
    ...
    vectors.push_back(
        GeneratedVector(GeneratedPoint(Pose(x_p, y_p, yaw), curvature),
                        linear_vel, linear_accel, linear_jerk));
  }
  return vectors;
}
...
find_max_accel(std::vector<SplineGenerator::GeneratedVector> vectors) {
  return std::max_element(vectors.begin(), vectors.end(),
...
SplineGenerator::gradient_descent(ControlVector &start, ControlVector &end,
                                  bool fast) {
  auto start_vel = 0.2;
  auto end_vel = 0.2;
  ...
  std::transform(vectors.begin(), vectors.end(), std::back_inserter(out),
...
std::vector<ProfilePoint> SplineGenerator::parameterize(
  ...
  // Forward Pass
  ConstrainedState predecessor(raw_path.front().pose, 0, 0, preferred_start_vel,
                               constraints.min_accel, constraints.max_accel);
  ...
  ConstrainedState successor(
      raw_path.back().pose, 0, constrainedStates.back().distance,
      preferred_end_vel, constraints.min_accel, constraints.max_accel);
...
void SplineGenerator::forward_pass(ConstrainedState *predecessor,
  ...
  while (true) {
    ...
    auto model_max_vel =
        model
            ->constraints(successor->pose, successor->curvature,
                          successor->max_vel)
            .max_vel;
...
void SplineGenerator::backward_pass(ConstrainedState *predecessor,
  ...
  while (predecessor->max_vel >
    ...
    auto model_max =
        model
            ->constraints(predecessor->pose, predecessor->curvature,
                          predecessor->max_vel)
            .max_vel;
...
SplineGenerator::get_point_at_time(const ControlVector start,
  ...
  return lerp_point(get_x_spline(start, end, duration),
                    get_y_spline(start, end, duration), *prev_sample, *sample,
                    i);
}
...
ProfilePoint SplineGenerator::lerp_point(QuinticPolynomial x_qp,
  ...
  return ProfilePoint(ControlVector(Pose(new_x, new_y, new_yaw), new_v,
                                    p_start.vector.accel, p_start.vector.jerk),
                      new_wheel_vels, new_curvature, new_t);
}
